...
bool CompleteMultiUploadReq::GenerateRequestBody(std::string* body) const {
  ...
  for (auto i = 0; i < part_numbers.size(); ++i) {
    ...
    part_node->append_node(doc.allocate_node(
        rapidxml::node_element, doc.allocate_string("PartNumber"),
        doc.allocate_string(
            StringUtil::Uint64ToString(part_numbers[i]).c_str())));
    part_node->append_node(
        doc.allocate_node(rapidxml::node_element, doc.allocate_string("ETag"),
...
std::map<std::string, std::string> CopyReq::GetInitHeader() const {
  ...
  for (; c_itr != m_headers_map.end(); ++c_itr) {
    if (c_itr->first == "Cache-Control" ||
        c_itr->first == "Content-Disposition" ||
        ...
        c_itr->first == "x-cos-grant-write" ||
        c_itr->first == "x-cos-grant-full-control" ||
...
bool DeleteObjectsReq::GenerateRequestBody(std::string* body) const {
  ...
  rapidxml::xml_node<>* root_node = doc.allocate_node(
      rapidxml::node_element, doc.allocate_string("Delete"), NULL);
  doc.append_node(root_node);
  root_node->append_node(
      doc.allocate_node(rapidxml::node_element, doc.allocate_string("Quiet"),
  ...
  for (std::vector<ObjectVersionPair>::const_iterator c_itr = m_objvers.begin();
    ...
    rapidxml::xml_node<>* object_node = doc.allocate_node(
        rapidxml::node_element, doc.allocate_string("Object"), NULL);
    ...
    if (!c_itr->m_version_id.empty()) {
      object_node->append_node(doc.allocate_node(
          rapidxml::node_element, doc.allocate_string("VersionId"),
...
bool PostObjectRestoreReq::GenerateRequestBody(std::string* body) const {
  ...
  rapidxml::xml_document<> doc;
  rapidxml::xml_node<>* root_node = doc.allocate_node(
      rapidxml::node_element, doc.allocate_string("RestoreRequest"), NULL);
  doc.append_node(root_node);
  ...
  rapidxml::xml_node<>* xml_rule = doc.allocate_node(
      rapidxml::node_element, doc.allocate_string("CASJobParameters"), NULL);
  root_node->append_node(xml_rule);
  xml_rule->append_node(doc.allocate_node(rapidxml::node_element,
                                          doc.allocate_string("Tier"),
...
bool SelectObjectContentReq::GenerateRequestBody(std::string* body) const {
  ...
  if (m_sql_expression.empty() || m_inputserialization.empty() ||
      m_outputserialization.empty() || m_expression_type != "SQL") {
  ...
  root_node = doc.allocate_node(rapidxml::node_element,
                                doc.allocate_string("SelectRequest"), NULL);
  doc.append_node(root_node);
  root_node->append_node(doc.allocate_node(
      rapidxml::node_element, doc.allocate_string("Expression"),
      doc.allocate_string(m_sql_expression.c_str())));
  root_node->append_node(doc.allocate_node(
      rapidxml::node_element, doc.allocate_string("ExpressionType"),
  ...
  request_progress = doc.allocate_node(
      rapidxml::node_element, doc.allocate_string("RequestProgress"), NULL);
  root_node->append_node(request_progress);
  request_progress->append_node(
      doc.allocate_node(rapidxml::node_element, doc.allocate_string("Enabled"),
...
bool PutLiveChannelReq::GenerateRequestBody(std::string* body) const {
  ...
  doc.append_node(root_node);
  root_node->append_node(doc.allocate_node(
      rapidxml::node_element, doc.allocate_string("Description"),
      doc.allocate_string(m_config.GetDescription().c_str())));
  root_node->append_node(
      doc.allocate_node(rapidxml::node_element, doc.allocate_string("Switch"),
                        doc.allocate_string(m_config.GetSwitch().c_str())));

  rapidxml::xml_node<>* target_node = doc.allocate_node(
      rapidxml::node_element, doc.allocate_string("Target"), NULL);
  target_node->append_node(
      doc.allocate_node(rapidxml::node_element, doc.allocate_string("Type"),
                        doc.allocate_string(m_config.GetType().c_str())));
  target_node->append_node(doc.allocate_node(
      rapidxml::node_element, doc.allocate_string("FragDuration"),
      doc.allocate_string(
          StringUtil::IntToString(m_config.GetFragDuration()).c_str())));
  target_node->append_node(doc.allocate_node(
      rapidxml::node_element, doc.allocate_string("FragCount"),
      doc.allocate_string(
          StringUtil::IntToString(m_config.GetFragCount()).c_str())));
  target_node->append_node(doc.allocate_node(
      rapidxml::node_element, doc.allocate_string("PlaylistName"),
      doc.allocate_string(m_config.GetPlaylistName().c_str())));
