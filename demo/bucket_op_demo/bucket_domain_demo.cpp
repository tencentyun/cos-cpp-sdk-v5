...
/**
 * 本样例演示了如何使用 COS C++ SDK 设置与查询存储桶绑定的自定义域名
 */
...
// 设置存储桶绑定自定义域名
void PutBucketDomainDemo(qcloud_cos::CosAPI& cos) {
...
// 获取存储桶绑定自定义域名
void GetBucketDomainDemo(qcloud_cos::CosAPI& cos) {
...
int main() {
    try {
        ...
        PutBucketDomainDemo(cos);
        GetBucketDomainDemo(cos);
