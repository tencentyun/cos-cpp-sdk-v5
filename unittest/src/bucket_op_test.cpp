    ...
    for (auto& bucketname : bucket_list) {
      ...
      for (std::vector<COSVersionSummary>::const_iterator c_itr = summs.begin();
        ...
        if (!del_result.IsSucc()) {
          std::cout << "DeleteObject Failed, check object=" << summ.m_key
                    << std::endl;
        }
      }
    ...
    for (int i = 0; i < 5; ++i) {
      {
        ...
        PutObjectByStreamReq req(m_bucket_name,
                                 prefixA + StringUtil::IntToString(i), iss);
...

      {
        ...
        PutObjectByStreamReq req(m_bucket_name,
                                 prefixB + StringUtil::IntToString(i), iss);
        req.SetXCosStorageClass(kStorageClassStandardIA);
        PutObjectByStreamResp resp;
        CosResult result = m_client->PutObject(req, &resp);
        EXPECT_TRUE(result.IsSucc());
...
#endif
  // std::string dest = "qcs:id/0:cos:" +
  // StringUtil::StringRemovePrefix(m_config2->GetRegion(), "cos.")
...
TEST_F(BucketOpTest, PutBucketLifecycleTest) {
  ...
  PutBucketLifecycleResp resp;
  {
    ...
    // Days in the Expiration action for filter must be greater than Days in the
    // Transition action
...
TEST_F(BucketOpTest, GetBucketLifecycleTest) {
  ...
  // 验证第一条rule
  {
    ...
    const LifecycleNonCurrExpiration& non_cur_expiration =
        rule.GetNonCurrExpiration();
    EXPECT_EQ(8, non_cur_expiration.GetDays());

    const LifecycleNonCurrTransition& non_cur_transition =
        rule.GetNonCurrTransition();
...
TEST_F(BucketOpTest, PutBucketACLTest) {
  ...
  // 1. Put
  {
    ...
    grant.m_grantee.m_display_name =
        "qcs::cam::uin/" + m_owner2 + ":uin/" + m_owner2;
...
TEST_F(BucketOpTest, GetBucketObjectVersions) {
  ...
  // 4. 获取Object版本
  // object_test
  // 有两个版本，object_test2有两个把呢不能（旧版本，最新版本标记删除）
  {
...
TEST_F(BucketOpTest, PutBucketInventory) {
  // 为存储桶配置清单,成功返回true,失败返回false
  {
    ...
    std::string target_bucket =
        "qcs::cos:" + m_config->GetRegion() + "::" + m_bucket_name;
...

  {
    ...
    for (; itr != inventory_vec.end(); ++itr) {
      ...
      if (itr->GetId() == "list2") {
        ...
        EXPECT_EQ("CSV", destination.GetFormat());
        // EXPECT_EQ("100010974951", destination.GetAccountId());
        // EXPECT_EQ("qcs::cos:ap-guangzhou::loggtest-1259743191",
        ...
        EXPECT_EQ("CSV", destination.GetFormat());
        // EXPECT_EQ("100010974951", destination.GetAccountId());
        // EXPECT_EQ("qcs::cos:ap-guangzhou::test3-1259743191",
        // destination.GetBucket());
        EXPECT_EQ(m_owner, destination.GetAccountId());
        EXPECT_EQ(target_bucket, destination.GetBucket());
        EXPECT_TRUE(destination.GetEncryption());
